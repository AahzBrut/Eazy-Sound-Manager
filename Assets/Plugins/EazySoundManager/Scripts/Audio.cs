...
namespace Hellmade
{
    ...
    public class Audio
    {
        ...
        {
            get => sourceTransform;
            set => sourceTransform = value == null ? _soundManager.gameObject.transform : value;
        }
        ...
        private readonly EazySoundManager _soundManager;

        public Audio(EazySoundManager soundManager, AudioType audioType, AudioClip clip, bool loop, bool persist, float volume, float fadeInValue, float fadeOutValue, Transform sourceTransform)
        {
            ...
            Pitch = 1;
            StereoPan = 0;
            if (sourceTransform != null && sourceTransform != _soundManager.gameObject.transform)
            {
                SpatialBlend = 1;
            }
            ReverbZoneMix = 1;
        ...
        public void Play(float volume)
        {
            ...
            if (Pooled)
            {
                ...
                var restoredFromPool = _soundManager.RestoreAudioFromPool(Type, AudioID);
                if (!restoredFromPool)
                {
                    return;
                }
        ...
        public void Update()
        {
            ...
            AudioSource.volume = Type switch
            {
                AudioType.Music => Volume * _soundManager.GlobalMusicVolume * _soundManager.GlobalVolume,
                AudioType.Sound => Volume * _soundManager.GlobalSoundsVolume * _soundManager.GlobalVolume,
                AudioType.UISound => Volume * _soundManager.GlobalUISoundsVolume * _soundManager.GlobalVolume,
