...
namespace Hellmade
{
    ...
    public class EazySoundManager : MonoBehaviour
    {
        ...
        {
            var audioDict = new Dictionary<int, Audio>();
            switch (audioType)
            {
        ...
        private bool GetAudioTypeIgnoreDuplicateSetting(Audio.AudioType audioType)
        {
            ...
            {
                Audio.AudioType.Music => IgnoreDuplicateMusic,
                Audio.AudioType.Sound => IgnoreDuplicateSounds,
                Audio.AudioType.UISound => IgnoreDuplicateUISounds,
                _ => false
            };
        }
        ...
        {
            var audio = GetMusicAudio(audioID);
            if (audio != null)
            {
                return audio;
            }
        ...
        {
            var audio = GetMusicAudio(audioClip);
            if (audio != null)
            {
                return audio;
            }
        ...
        private Audio GetAudio(Audio.AudioType audioType, bool usePool, AudioClip audioClip)
        {
            ...
            var audioTypeKeys = new List<int>(audioDict.Keys);
            var poolKeys = new List<int>(audioPool.Keys);
            var keys = usePool ? audioTypeKeys.Concat(poolKeys).ToList() : audioTypeKeys;
            foreach (var key in keys)
            {
                Audio audio = null;
        ...
        private int PrepareAudio(Audio.AudioType audioType, AudioClip clip, float volume, bool loop, bool persist, float fadeInSeconds, float fadeOutSeconds, float currentMusicfadeOutSeconds, Transform sourceTransform)
        {
            ...
            var audioDict = GetAudioTypeDictionary(audioType);
            var ignoreDuplicateAudio = GetAudioTypeIgnoreDuplicateSetting(audioType);

            if (ignoreDuplicateAudio)
            {
                var duplicateAudio = GetAudio(audioType, true, clip);
            ...
            var audio = new Audio(this, audioType, clip, loop, persist, volume, fadeInSeconds, fadeOutSeconds, sourceTransform);
        ...
        {
            var audioID = PrepareAudio(audioType, clip, volume, loop, persist, fadeInSeconds, fadeOutSeconds, currentMusicfadeOutSeconds, sourceTransform);
